...
void followObject(float objectWidth, float objectHeight, float x_pos, float speed_coefficient, cv::Mat cvHomography)
{
...
void objectCallback(const std_msgs::Float32MultiArrayPtr &object)
{
   if (object->data.size() > 0)
   {
      ...
      if (id < 8)
      {
         followObject(objectWidth, objectHeight, x_pos, speed_coefficient, cvHomography);	   
      }
      else
      {
      	set_vel.linear.x = 0;
